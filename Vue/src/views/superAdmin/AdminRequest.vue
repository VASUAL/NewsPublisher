<template>
  <span class="sp1">提示：同意管理员申请之后该用户身份将变为管理员</span>
    ...
    <el-table :data="tableData" border class="el1">
      ...
      <el-table-column label="操作" width="150" align="center" key="slot">
        ...
        <template #default="scope" >
          <el-button type="text" size="big" @click="this.upRole(scope.row)">同意</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页组件-->
  ...
  <!--同意申请弹窗，点击同意时才显示-->
  <el-dialog v-model="dialogFormVisibleForUpRole" title="管理员申请">
    <span>是否同意该用户的管理员申请？</span>
    <template #footer>
      <span class="dialog-footer">
        <el-button @click="dialogFormVisibleForUpRole = false">取消</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      pageSize: 5,     //每页显示的条数
      currentPage: 1,  //当前所在页
      dialogFormVisibleForUpRole: false
    }
  },
  methods: {
    //加载表格数据（结合分页操作，因此加载数据时需要将pageSize和currentPage传到后端）
    loadData() {
      request.get("/user/getAllAdminRequest",{
        params: {
    ...
    upRole(row) {
      rowForEdit = row
      this.dialogFormVisibleForUpRole = true
    },
    //确认申请
    conform() {
      request.post("/user/upRole",rowForEdit.email).then(res =>{
        this.loadData()
        this.dialogFormVisibleForUpRole = false
      })
    },
